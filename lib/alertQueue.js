...
class AlertQueue {
  ...
  upsert (path, priority, message) {
    ...
    // A priority increase on an already-seen alert (e.g. alert manager
    // escalating an unacknowledged warning to alarm) is treated as a
    // fresh occurrence for announcement purposes: resetting
    // lastAnnounced makes _playNext() treat it as "never announced yet"
    // so it gets picked up as soon as it's not preempted by something
    // still more urgent, rather than waiting out the rest of the normal
    // repeat interval (see docs/alerts-only-plan.md, escalation decision).
    ...
    const entry = {
      ...
      lastAnnounced: existing && !isEscalation ? existing.lastAnnounced : null
    }
