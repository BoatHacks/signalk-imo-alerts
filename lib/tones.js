...
// Same safety net as lib/tts.js's SPAWN_TIMEOUT_MS - see there for why
// (Windows can leave a spawned child neither erroring nor exiting).
const SPAWN_TIMEOUT_MS = 10000

// Same as lib/tts.js's safeKill - child.kill() can itself throw
// synchronously (EINVAL) on Windows when the underlying process handle
// never fully started. Confirmed root cause of a Windows CI hang: an
// uncaught EINVAL here, thrown synchronously from inside an alerts.*
// delta-handling callback chain (interruptPlayback -> stop -> kill),
// corrupted the test process enough that it never exited afterward.
...
// Short human-readable descriptions, e.g. for the test-mode webapp's tone
// dropdown - see docs/design.md, "Alert tone patterns" for the full detail.
const TONE_CODE_DESCRIPTION = Object.freeze({
  [TONE_CODE.GENERAL_EMERGENCY]: 'General emergency alarm (7 short + 1 long blast)',
  [TONE_CODE.CONTINUOUS]: 'Continuous tone',
  [TONE_CODE.SQUARE_PULSE]: 'Square pulse train',
  [TONE_CODE.SAWTOOTH]: 'Sawtooth rise, sharp fall',
  [TONE_CODE.CLUSTERED_PULSES]: 'Clustered short pulses',
  [TONE_CODE.SINE_FM]: 'Sinusoidal frequency modulation'
})
...
/**
 * Resolve the actual clip file to play for an alert: a muster-list text
 * pattern override (highest precedence), a user-configured per-priority
 * override (preset built-in code or a custom pattern), or the built-in
 * default for that priority.
 * @param {number} priority
 * @param {string} notificationPath
 * @param {Array<{path: string, zone?: string, pattern: string}>} [musterListCodes]
 * @param {object} [priorityToneConfig] - keyed by PRIORITY value, e.g.
 *   { [PRIORITY.CAUTION]: { preset: '3a' | ... | 'custom', pattern: '...' } }.
 *   Currently only meaningful for Caution/Warning - see docs/design.md,
 *   "Priority -> default tone code mapping" for why those two have no
 *   Table 7.2 basis and are fully user-configurable as a result.
 * @returns {string|null} absolute path to a .wav file, or null if none applies
 */
function resolveClipPath (priority, notificationPath, musterListCodes = [], priorityToneConfig = {}) {
  const musterOverride = musterListCodes.find((m) => m.path === notificationPath)
  if (musterOverride) {
    return resolveMusterClipPath(musterOverride.pattern)
  }

  const override = priorityToneConfig[priority]
  if (override && override.preset === 'custom' && override.pattern) {
    return resolveMusterClipPath(override.pattern)
  }
  if (override && override.preset && override.preset !== 'custom') {
    return clipPathFor(override.preset)
  }
...
function play (clipPath, spawnFn = spawn) {
  ...
  const promise = new Promise((resolve) => {
    ...
    const timer = setTimeout(() => {
      safeKill(child)
      settle({ played: false, reason: 'player timed out' })
    }, SPAWN_TIMEOUT_MS)

    try {
      child = spawnFn('aplay', [clipPath])
    } catch (err) {
      settle({ played: false, reason: `player not available: ${err.message}` })
      return
    }
    child.on('error', (err) => {
      settle({ played: false, reason: `player failed: ${err.message}` })
    })
    child.on('exit', (code) => {
      settle({ played: code === 0, reason: code === 0 ? null : `player exited with code ${code}` })
    })
  })
